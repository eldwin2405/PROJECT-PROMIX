...
COUNTER_NAMESPACE = "promix-reader-eldwin"
COUNTER_PREFIX = "daily-visitors"


def get_daily_visitors() -> int:
    today_str = datetime.now(ZoneInfo("Asia/Jakarta")).strftime("%Y-%m-%d")
    counter_key = f"{COUNTER_PREFIX}-{today_str}"

    namespace = quote(COUNTER_NAMESPACE, safe="")
    key = quote(counter_key, safe="")

    session_key = f"visitor_counted_{today_str}"

    if session_key not in st.session_state:
        with urlopen(f"https://api.counterapi.dev/v1/workspaces/{namespace}/counters/{key}/up") as response:
            data = json.load(response)

        st.session_state[session_key] = True
        st.session_state["daily_visitors"] = int(data["value"])
    else:
        with urlopen(f"https://api.counterapi.dev/v1/workspaces/{namespace}/counters/{key}") as response:
            data = json.load(response)

        st.session_state["daily_visitors"] = int(data["value"])

    return st.session_state["daily_visitors"]
...
st.metric("Total Pengunjung Hari Ini", daily_visitors)
...
if uploaded_file is not None:
    try:
        ...
        if show_only_beverage_totals and "Beverages" in frames:
            beverage_totals = {
            "Air Mineral",
            "TOTAL LEMON TEA",
            "TOTAL ORANGE",
            ...
            beverage_df = frames["Beverages"].copy()
            frames["Beverages"] = beverage_df[
            beverage_df["Menu"].isin(beverage_totals)
            ]

        elif hide_beverage_totals and "Beverages" in frames:
            beverage_df = frames["Beverages"].copy()
            frames["Beverages"] = beverage_df[
